...
const CustomerDetail = () => {
  ...
  const activeStoreId = storeId === 'admin' ? (c?.storeId || stores?.[0]?.id || 1) : Number(storeId);
  ...
  const { createRepairAsync } = useRepairMutations();
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:p-8">
        ...
        {activeTab === 'info' && (
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-3 text-xs text-slate-405 font-bold px-1">
          ...
          <div>
            {!c.orders?.length ? (
              ...
              <>
                ...
                <div className="hidden md:block border border-slate-100 rounded-xl overflow-x-auto shadow-sm animate-fade-in">
                              ...
                              <div className="flex flex-col gap-1 items-start">
                                <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-bold border ${
                                  isOrderInWarranty(order.date, getOrderWarrantyMonths(order))
                                    ...
                                    : 'text-slate-605 bg-slate-105 border-slate-200'
                                }`}>
                    ...
                    return (
                      <div key={order.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 space-y-3 hover:shadow-md transition-all">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className="text-[10px] bg-slate-100 font-bold px-2 py-0.5 rounded text-slate-505">
                            {paymentMethod}
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 text-xs border-b border-slate-50 pb-2">
                          <div>
                            ...
                            <p className="font-bold text-slate-705">{qty} × {fmt(unitPrice)}</p>
                          </div>
                          <div>
                ...
                <div className="p-5 sm:p-6 bg-white border border-slate-100 rounded-2xl shadow-sm">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                    {[
                      ...
                      <div key={label} className="bg-slate-55 p-3 sm:p-4 rounded-xl border border-slate-100 text-center">
                ...
                <div className="p-5 sm:p-6 bg-white border border-slate-100 rounded-2xl shadow-sm">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      ...
                      <div key={lbl} className="bg-slate-55 p-4 rounded-xl border border-slate-100/65 shadow-sm">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">{lbl}</p>
                        ...
                        return (
                          <div key={idx} className="bg-slate-50 rounded-2xl border border-slate-150 p-5 space-y-4">
                            <div className="flex justify-between items-center flex-wrap gap-2 border-b border-slate-200 pb-2.5">
                              ...
                              <span className="text-[9px] bg-slate-200 text-slate-655 font-mono font-bold px-2 py-0.5 rounded">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              ...
                              {/* Left Eye */}
                              <div className="bg-white p-4 rounded-xl border border-slate-205 shadow-sm space-y-2">
                  ...
                  {c.history.map((item, idx) => {
                    ...
                    return (
                      <div key={idx} className="relative flex items-start gap-4 pl-0 py-3 group">
                        ...
                        {/* Content */}
                        <div className="flex-1 min-w-0 bg-slate-50 border border-slate-100 rounded-2xl p-4 hover:bg-slate-100 transition-colors">
                          <div className="flex items-start justify-between gap-2 flex-wrap">
                            <p className="text-sm font-bold text-slate-900">{item.event}</p>
                            <span className="text-xs font-semibold text-slate-450 whitespace-nowrap">{fmtDate(item.date)}</span>
                          </div>
            ...
            {repairsLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {claimsList.map(claim => (
                  <div key={claim.id} className="p-5 bg-slate-50 border border-slate-100 rounded-2xl relative overflow-hidden transition-all hover:bg-slate-100/50 animate-fade-in">
  ...
  const inputCls = (f) =>
    `w-full px-3 py-2.5 text-sm font-medium rounded-xl border transition-all focus:outline-none focus:ring-4 bg-white ${
      errors[f]
        ...
        : 'border-slate-205 focus:ring-blue-500/10 focus:border-blue-500 placeholder:text-slate-400'
    }`;
        ...
        <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-slate-100 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-550">File a new warranty repair or service claim</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-5 sm:px-6 py-5 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-semibold text-slate-605 mb-1.5 flex items-center gap-1.5">
          ...
          <div className="px-5 sm:px-6 py-4 border-t border-slate-100 flex items-center justify-end gap-3 flex-shrink-0 bg-slate-50">
            <button type="button" onClick={onClose}
              className="px-4 py-2 text-sm font-semibold text-slate-650 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
              Cancel
            </button>
            <button type="submit"
