...
import { MOCK_SUPPLIER_PAYMENTS, ACCOUNTANT_STORES } from '../../data/accountantData';
...
const SupplierPayments = () => {
  ...
  const branchFiltered = useMemo(() => {
    if (!selectedStore || selectedStore.id === 'all' || selectedStore.isAll) return MOCK_SUPPLIER_PAYMENTS;
    return MOCK_SUPPLIER_PAYMENTS.filter(s => s.storeName === selectedStore.store_name);
  }, [selectedStore]);
  ...
  const {
    ...
    searchKeys: ['supplierName', 'contactPerson', 'storeName'],
    itemsPerPage: 10,
  ...
  const columns = [
    { key: 'supplierName', header: 'Supplier', sortable: true, render: (row) => <span className="font-bold text-slate-800">{row.supplierName}</span> },
    { key: 'contactPerson', header: 'Invoice', sortable: true, render: (row) => <span className="font-semibold text-slate-600">{row.contactPerson}</span> },
    { key: 'totalBilled', header: 'Purchase Amount', sortable: true, render: (row) => <span className="font-semibold text-slate-700">{formatRupee(row.totalBilled)}</span> },
    ...
    { key: 'dueAmount', header: 'Pending Amount', sortable: true, render: (row) => <span className={`font-bold ${row.dueAmount > 0 ? 'text-rose-600' : 'text-slate-400'}`}>{formatRupee(row.dueAmount)}</span> },
    ...
    <div className="bg-white rounded-2xl border border-slate-200/60 p-4 shadow-sm space-y-3 active:scale-[0.99] transition-transform cursor-pointer">
      <div className="flex justify-between items-start gap-2 border-b border-slate-100 pb-2">
        <div className="min-w-0 flex-1">
          <p className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider">Supplier</p>
          <p className="font-bold text-slate-800 text-sm truncate">{row.supplierName}</p>
        </div>
        <StatusBadge status={row.status} />
      </div>
      <div className="flex justify-between items-center text-xs">
        <span className="text-slate-400 font-bold">Contact</span>
        <span className="font-semibold text-slate-600 truncate ml-2">{row.contactPerson}</span>
      </div>
      <div className="flex justify-between items-center text-xs">
      ...
      <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-100">
        <div>
          <p className="text-[9px] text-slate-400 font-bold">Purchase</p>
          <p className="text-xs font-bold text-slate-800">{formatRupee(row.totalBilled)}</p>
        </div>
        <div>
    ...
    <div className="p-3 sm:p-5 lg:p-8 max-w-[1600px] mx-auto animate-fade-in font-sans space-y-4 sm:space-y-6 min-w-0 overflow-x-hidden">
      <div>
        <div className="flex items-center gap-2 mb-1">
          <Truck className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-500 flex-shrink-0" />
          <h1 className="text-lg sm:text-xl lg:text-2xl font-black text-slate-900 tracking-tight">Supplier Payments</h1>
        </div>
        <p className="text-slate-500 text-[11px] sm:text-xs lg:text-sm font-semibold">Track supplier invoices, paid and pending amounts.</p>
      </div>
      ...
      <DataTable columns={columns} data={paginatedData} onRowClick={(row) => setDetailItem(row)} sortConfig={sortConfig} onSort={requestSort} emptyTitle="No supplier payments found" emptyDescription="All supplier balances are settled." mobileCardRender={mobileCardRender} />
      <Pagination totalItems={totalItems} itemsPerPage={itemsPerPage} currentPage={currentPage} onPageChange={setCurrentPage} />

      <DetailDrawer isOpen={!!detailItem} onClose={() => setDetailItem(null)} title="Supplier Payment Details">
        {detailItem && (
          <div className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <DetailField label="Supplier" value={detailItem.supplierName} />
              <DetailField label="Contact Person" value={detailItem.contactPerson} />
            ...
            <div className="grid grid-cols-2 gap-4">
              <DetailField label="Total Billed" value={formatRupee(detailItem.totalBilled)} />
              <DetailField label="Paid Amount" value={formatRupee(detailItem.paidAmount)} />
              <DetailField label="Pending Amount" value={formatRupee(detailItem.dueAmount)} danger={detailItem.dueAmount > 0} />
            </div>
            <hr className="border-slate-100" />
