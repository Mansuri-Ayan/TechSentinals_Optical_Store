...
const ShopkeeperSuppliers = () => {
  ...
  return (
    <PermissionGuard permission="suppliers:read" fallback={
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-50">
                  {displayedPurchaseOrders.map(po => (
                    <tr
                      key={po.id}
                      onClick={() => navigate(`/shopkeeper/suppliers/${po.supplier_id}`)}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 mb-6 animate-fade-in">
            {displayedSuppliers.map(s => (
              <SupplierCard
                key={s.id}
                supplier={s}
                onClick={() => navigate(`/shopkeeper/suppliers/${s.id}`)}
...
export default ShopkeeperSuppliers;
