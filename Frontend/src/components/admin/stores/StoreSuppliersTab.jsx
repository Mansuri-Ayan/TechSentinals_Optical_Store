...
const StoreSuppliersTab = ({ store }) => {
  const [searchTerm, setSearchTerm] = useState('');

  const { suppliers, isLoadingSuppliers, isSuppliersError } = useSuppliers(store.id, {
    limit: 100,
  });
  ...
  const filteredSuppliers = useMemo(() => {
    return suppliers.filter(s => {
      ...
      const person = (s.contact_person || '').toLowerCase();
      const city = (s.city || '').toLowerCase();
      const query = searchTerm.toLowerCase().trim();

      return !query || name.includes(query) || person.includes(query) || city.includes(query);
    });
