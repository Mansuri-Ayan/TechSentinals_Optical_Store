...
const Categories = () => {
  ...
  const {
    ...
    deleteCategoryAsync,
    isSavingCategory,
  } = useShopkeeperCategories(storeId, {
    page: catPage,
  ...
  const handleViewCategoryItems = (categoryId) => {
    navigate(`/shopkeeper/inventory?category_id=${categoryId}`);
  };

  const handleViewSubcategoryItems = (subcategoryId) => {
    navigate(`/shopkeeper/inventory?category_id=${selectedCategory.id}&subcategory_id=${subcategoryId}`);
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
                <Layers className="w-8 h-8 text-emerald-505" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {categories.map((cat) => {
                  ...
                  return (
                    ...
                    >
                      <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-slate-50 to-transparent rounded-bl-full opacity-55 pointer-events-none group-hover:scale-110 transition-transform duration-500" />
                      ...
                      <div className="flex items-center gap-4 mb-6">
                        <div className="flex-1 bg-slate-50 border border-slate-100 rounded-xl px-3 py-2 text-center">
                          <p className="text-xs text-slate-505 font-medium">Options</p>
                      ...
                      <div className="flex items-center gap-2 w-full pt-4 border-t border-slate-100">
                        ...
                        <button
                          ...
                          className={`flex-1 flex items-center justify-center gap-1.5 py-2 ${theme.bg} ${theme.color} rounded-xl text-xs font-bold hover:opacity-85 transition-colors border ${theme.border}`}
                        >
        ...
        <div className="animate-fade-in">
          ...
          {isLoadingSubcategories ? (
            ...
            <div className="bg-white border border-dashed border-slate-200 rounded-2xl p-12 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-slate-505 text-sm mb-4">Try adjusting your search or add a new option.</p>
            </div>
          ) : (
            <div className="relative">
